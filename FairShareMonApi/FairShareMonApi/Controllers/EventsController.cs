...
public class EventsController(IEventsService eventsService, IStatsService statsService, IExportService exportService, IWalletQrService walletQrService, IEventShareService shareService, IStringLocalizer<StringResources> localizer) : AppController
{
    ...
    [SwaggerOperation(
        ...
        Description = "Trả về cân bằng nợ của một đợt: với mỗi thành viên tham gia (người trả phiếu hoặc người gánh, kể cả thành viên đại diện ở mức 0đ và thành viên đã xóa mềm) là số tiền đã ứng, phải gánh và cân bằng (= đã ứng - phải gánh). Tổng cân bằng của tất cả thành viên luôn bằng 0. Xem được cho cả đợt đang mở và đã chốt; bỏ qua trạng thái đã trả. Đợt chưa có phiếu -> danh sách rỗng. Mỗi dòng còn kèm cờ isEligibleForAutoCascade cho biết đánh dấu \"đã trả\" ở cấp đợt cho thành viên đó có tự động lan xuống mọi phần gánh của họ hay không, số tiền đã tất toán lũy kế (clearedAmount, cộng dồn từ việc đánh dấu đã trả từng phần gánh/phiếu riêng lẻ), số tiền còn nợ thực tế (outstanding = phải nợ ròng - đã tất toán) và trạng thái tất toán (settlementStatus: \"Unsettled\"/\"PartiallySettled\"/\"Settled\").")]
